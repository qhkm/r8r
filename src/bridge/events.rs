...
//! - **r8r -> ZeptoClaw**: approval requests, execution results, health status
//! - **ZeptoClaw -> r8r**: approval decisions, workflow triggers, health pings
//!
//! The `type` field lives on [`BridgeEventEnvelope`], not on the event data itself.
//! This avoids `#[serde(untagged)]` pitfalls where overlapping fields cause silent
//! misdeserialization.
